...
from definitions import V3_version_letters, CBIO_CANCER_MUTATIONS_P, DISORDERED_THRESHOLD
...
def process_cancer_file(file_idx):
    # 1. Setup Folder
    cancer_files = sorted([f for f in os.listdir(CBIO_CANCER_MUTATIONS_P) if f.endswith('.csv')])     # TODO check that is the right folder
    ...
    file_path = os.path.join(CBIO_CANCER_MUTATIONS_P, target_file)
