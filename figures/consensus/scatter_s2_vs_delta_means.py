...
from definitions import RESULTS_DISTANCES_P, FILTERED_RESULTS_DISTANCES_P, COLOR_MAP
...
for run in RUNS[::-1]:
    ...
    sc2 = ax1b.scatter(
        ...
        cmap=CMAP_NAME, norm=norm,
        s=240, alpha=1.0,
        linewidths=0.75, edgecolors="dimgray",
        zorder=3,
    )

    _thresh  = -np.log10(0.05)
    _y_top   = ax1b.get_ylim()[1]
    _alpha_b   = 0.28
    _alpha_p   = 0.35
    _N       = 256

    # Grey flat band below threshold
    ax1b.fill_between([-0.05, 0.175], 0, _thresh,
                      color=COLOR_MAP["grey"], alpha=0.17, zorder=0, linewidth=0)

    # Benign gradient: full color at x=-0.05, transparent at x=0
    _bc   = mcolors.to_rgba(COLOR_MAP["benign"])
    _grad = np.zeros((1, _N, 4))
    _grad[0, :, :3] = _bc[:3]
    _grad[0, :, 3]  = np.linspace(_alpha_b, 0, _N)
    ax1b.imshow(_grad, aspect="auto", origin="upper",
                extent=[-0.05, 0, _thresh, _y_top], zorder=0)

    # Pathogenic gradient: transparent at x=0, full color at x=0.175
    _pc   = mcolors.to_rgba(COLOR_MAP["pathogenic"])
    _grad = np.zeros((1, _N, 4))
    _grad[0, :, :3] = _pc[:3]
    _grad[0, :, 3]  = np.linspace(0, _alpha_p, _N)
    ax1b.imshow(_grad, aspect="auto", origin="upper",
                extent=[0, 0.175, _thresh, _y_top], zorder=0)

    ax1b.set_ylim(0, _y_top)

    ax1b.set_xlabel("𝔼(Cancer)-𝔼(Background)", fontsize=20)
    ax1b.set_ylabel("−log₁₀(q-value)", fontsize=20)
    ax1b.tick_params(labelsize=14)
    ax1b.spines[["top", "right"]].set_visible(False)
    ax1b.grid(alpha=0.0, linestyle="--")                                            # no grid lines

    ax1b.set_xlim(-0.05, 0.175)

    cbar2 = fig1b.colorbar(sc2, ax=ax1b, pad=0.02, fraction=0.04)
    cbar2.set_label("Mean Consensus Score", fontsize=18)

    cbar2.set_ticks([])

    cbar2.ax.tick_params(labelsize=10)

    ax1b.axhline(-np.log10(0.05), color="#555555", linewidth=2.0, linestyle="--", alpha=0.6)
    ax1b.text(
        0.175 - 0.035, -np.log10(0.05) + 0.08,
        "q = 0.05", color="#555555", fontsize=18, ha="left", va="bottom",
        bbox=dict(boxstyle="round,pad=0.3", fc="white", alpha=0.85, ec="#CCCCCC")
    )

    ax1b.axvline(0, color="#555555", linewidth=2.0, linestyle="--", alpha=0.6)

    grey_patch2 = mpatches.Patch(
        facecolor="#BBBBBB",
        label=f"Not in Bailey et al. 2018 {cancer_label}  (n={len(grey_df)})"
    )
    ax1b.legend(handles=[grey_patch2], fontsize=14, loc="lower right",
                framealpha=0.85, edgecolor="#CCCCCC")
