...
if __name__ == "__main__":
    ...
    for pathway_id, pathway_label, nature, col_bg, col_cancer, suffix in B1_PATHWAYS:
        ...
        ax.stairs(hist_bg, bin_edges, color=col_bg, fill=True, alpha=0.6,
                  label="Expected (PSSM-Weighted BG)")
        ax.stairs(hist_bg, bin_edges, color=col_bg, linewidth=1.5, alpha=0.8)

        if hist_cancer.sum() > 0:
            ax.stairs(hist_cancer, bin_edges, color=col_cancer, fill=True, alpha=0.4,
        ...
        # ----------- one more version - simplified
        fig, ax = plt.subplots(figsize=(3.8, 2.8))
        fig.suptitle(None)

        ax.stairs(hist_bg, bin_edges, color=col_bg, fill=True, alpha=0.7,
                  label="Background")
        ax.stairs(hist_bg, bin_edges, color=col_bg, linewidth=1.5, alpha=0.85)

        if hist_cancer.sum() > 0:
            ax.stairs(hist_cancer, bin_edges, color=col_cancer, fill=True, alpha=0.5,
                      label=f"Cancer")
            ax.stairs(hist_cancer, bin_edges, color=col_cancer, linewidth=1.5)
        else:
            print(f"    Warning: no cancer mutations found for {pathway_id} / {CANCER_NAME}")

        ax.set_xlabel("Pathogenicity", labelpad=3)
        ax.set_ylabel(None)
        ax.set_xlim(0, 1)
        y_max = max(hist_bg.max(), hist_cancer.max() if hist_cancer.sum() > 0 else 0) * 1.3
        ax.set_ylim(0, y_max)
        ...
        # Dashed vertical mean lines
        ax.axvline(mu_bg, color=col_bg, linestyle='--', linewidth=1.2, alpha=0.95)
        ax.axvline(mu_cancer, color=col_cancer, linestyle='--', linewidth=1.2, alpha=0.95)
        ...
        fig.tight_layout()
        out_name = f"figure_2_simple_{suffix}.png"
        fig.savefig(OUTPUT_DIR / out_name)
        plt.close(fig)
        print(f"    Saved {out_name}")
